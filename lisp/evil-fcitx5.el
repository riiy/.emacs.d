; evil-fcitx5.el -- providing auto toggling on the top of fcitx5-remote
...
(defun
  get-current-IME
  ()
  (if (equal (shell-command-to-string "fcitx5-remote") user-IME-ID) "user-IME" "default-IME"))

(defvar IME-state (concat (get-current-IME) "-state"))

(defun
  switch-to-default-IME ()
  (when
    (not (equal (get-current-IME) "default-IME"))
...
(defun
  switch-to-user-IME ()
  (when
    (not (equal (get-current-IME) "user-IME"))
...
(defun
  ...
  (when
    ...
    (setq mode-line-format (delq 'IME-state-mode-line-tag mode-line-format))
    ; just like the evil-refresh-mode-line
    (let
      (
        (mlpos mode-line-format)
        pred)
      ...
      (while
        ...
        (setq pred mlpos mlpos (cdr mlpos)))
      (cond
        ((not mlpos))
        ((not nil)
...
(defun
  exit-insert-evil-fcitx5 () "When exit insert with user-IME, remember it"
  (if
...
(add-hook 'evil-insert-state-exit-hook 'exit-insert-evil-fcitx5)

(defun
  entry-insert-evil-fcitx5 () "When entry insert with user-IME-state, restore the user-IME"
  ...
  (progn (setq IME-state-mode-line-tag "[--]") (force-mode-line-update))
  (if
    (and (equal IME-state "user-IME-state"))
    (progn
      (switch-to-user-IME)
      ; print-msg
      (message "IME state Restored"))))

(add-hook 'evil-insert-state-entry-hook 'entry-insert-evil-fcitx5)
...
(defun
  ...
  (if
    (or (equal IME-state "user-IME-state") (equal (get-current-IME) "user-IME"))
    (progn
      (switch-to-default-IME) (IME-state-setter "default-IME-state")
      ;(evil-normal-state)
      (message "IME-state cleared"))))

(global-set-key (kbd force-back-to-default-state-key) 'force-back-to-default-state)
...
(defadvice
  evil-find-char
  (after change-to-user-IME-after-find-char (&optional count char))
  (switch-to-default-IME))
(ad-activate 'evil-find-char)

(defadvice
  evil-find-char-backward
  (after change-to-user-IME-after-rfind-char (&optional count char))
  (switch-to-default-IME))
(ad-activate 'evil-find-char-backward)

(defadvice
  evil-search-forward
  (after change-to-user-IME-state-after-search ())
  (switch-to-default-IME))
(ad-activate 'evil-search-forward)

(defadvice
  evil-search-backward
  (after change-to-user-IME-state-after-rsearch ())
  (switch-to-default-IME))
(ad-activate 'evil-search-backward)

(defadvice
  evil-replace
  (after change-to-user-IME-state-after-find-char (beg end &optional type char))
  (switch-to-default-IME))
